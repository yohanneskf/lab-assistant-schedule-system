...
export default function SchedulesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const conflictingSchedule = schedules.find(
      (schedule) =>
        ...
        ((schedule.labRoomId === formData.labRoomId &&
          schedule.timeSlotId === formData.timeSlotId) ||
          (schedule.labAssistantId === formData.labAssistantId &&
            schedule.timeSlotId === formData.timeSlotId))
    );
    ...
    if (editingSchedule) {
      const updated = updateScheduleAssignment(
        editingSchedule.id,
        scheduleData
      );
      if (updated) {
  ...
  const availableGroups = formData.sectionId
    ? getGroupsBySection(formData.sectionId)
    : [];
